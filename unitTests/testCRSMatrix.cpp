...
void emptyDeviceTest(ViewType<T const, COL_TYPE const> const & m)
{
  ...
  forall(gpu(), 0, numRows,
    [=] __device__ (INDEX_TYPE row)
    {
      for (INDEX_TYPE i = 0; i < m.numNonZeros(row); ++i)
      {
        LVARRAY_ERROR_IF(m.empty(row, COL_TYPE(2 * i)), "Row should contain even columns.");
        LVARRAY_ERROR_IF(!m.empty(row, COL_TYPE(2 * i + 1)), "Row should not contain odd columns.");       
      }
    }
  );
}
...
void sparsityPatternViewDeviceTest(ViewType<T const, COL_TYPE const> const & m)
{
  ...
  forall(gpu(), 0, numRows,
    ...
    {
      INDEX_TYPE const nnz = m.numNonZeros(row);
      LVARRAY_ERROR_IF(nnz != spView.numNonZeros(row), "The number of non zeros should be equal!");

      COL_TYPE const * const columns = m.getColumns(row);
      COL_TYPE const * const spColumns = spView.getColumns(row);
      LVARRAY_ERROR_IF(columns != spColumns, "The column pointers should be equal!");

      for (INDEX_TYPE i = 0; i < nnz; ++i)
      {
        LVARRAY_ERROR_IF(columns[i] != spColumns[i], "Columns should be equal!");
        LVARRAY_ERROR_IF(m.empty(row, columns[i]) != spView.empty(row, columns[i]), "Should be equal.");
        LVARRAY_ERROR_IF(m.empty(row, COL_TYPE(i)) != spView.empty(row, COL_TYPE(i)), "Should be equal.");
      }
    }
  );
