             ...
             std::array< SRC_INDEX_TYPE, N_SRC_INDICES > const & srcIndices )
{
  ...
  LVARRAY_ERROR_IF_NE_MSG( dst.getPreviousSpace(), MemorySpace::host, "%s", "Without Umpire only host memory is supported." );
  LVARRAY_ERROR_IF_NE_MSG( src.getPreviousSpace(), MemorySpace::host, "%s", "Without Umpire only host memory is supported." );
#endif
